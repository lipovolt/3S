...
<dl>
	...
	<div id="ProductInfo" class="main">
		<div class="tab-content">	
			<div class="sidenav-hd"><strong>EUB 邮费设置</strong></div>
			<form method="POST" id="updateFirstClass" name="updateFirstClass" action="<?php echo U('Szsw/Postage/save');?>">
			<div class="block-outer-hd" align="right">
				<button class="btn btn-blue btn-s" id="savePurchaseItemInfo">保存</button>
				<a class="btn btn-blue btn-s" href="<?php echo U('Szsw/Postage/eub',array('insert'=>true));?>">新增</a>
		    </div>
			<table id="tablelist" class="tablelist">
				...
				<tr>
					<?php if(is_array($postage)): $i = 0; $__LIST__ = $postage;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						<td>
							<input type="hidden" id="id<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_ID')]); ?>" name="id<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_ID')]); ?>" value="<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_ID')]); ?>"> 
							<input type="text" id="country<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_ID')]); ?>" name="country<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_ID')]); ?>" value="<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_COUNTRY')]); ?>" style="width:60px;"/> 
						</td>
						<td><input type="text" id="register<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_ID')]); ?>" name="register<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_ID')]); ?>" value="<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_REGISTER')]); ?>" style="width:60px;"/> </td>
						<td><input type="text" id="fee<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_ID')]); ?>" name="fee<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_ID')]); ?>" value="<?php echo ($vo[C('DB_SZ_POSTAGE_EUB_FEE')]); ?>" style="width:60px;"/> </td>
