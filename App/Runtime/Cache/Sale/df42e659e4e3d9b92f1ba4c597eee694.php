...
function save(){
	...
	form.action="<?php echo U('Sale/SzSale/updateSzSalePlan');?>"; 
	form.submit();
}
</script>
</head>
...
<dl>
	...
	<div id="ProductInfo" class="main">
		<form name="search_product" id="search_product" action="<?php echo U('Sale/SzSale/szSaleSuggest');?>" method="POST">
		...
		<div>
			<div class="tab-content">
				...
				<a class="btn btn-s btn-blue" href="<?php echo U('Sale/SzSale/calSzUSSaleInfo');?>">更新建议</a>
				...
				<table id="tablelist" class="tablelist">
					...
					<?php if(is_array($suggest)): $i = 0; $__LIST__ = $suggest;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><div class="tl">
						<input type="hidden" id="id-<?php echo ($vo[C('DB_USSW_SALE_PLAN_ID')]); ?>" name="id-<?php echo ($vo[C('DB_USSW_SALE_PLAN_ID')]); ?>" value="<?php echo ($vo[C('DB_USSW_SALE_PLAN_ID')]); ?>" />
						<input id="sale_price-<?php echo ($vo[C('DB_USSW_SALE_PLAN_ID')]); ?>" name='sale_price-<?php echo ($vo[C('DB_USSW_SALE_PLAN_ID')]); ?>' type='text' value='<?php echo ($vo[C('DB_USSW_SALE_PLAN_PRICE')]); ?>' style="width:60px;"/></div></td>
						...
						<?php if(($vo[C('DB_USSW_SALE_PLAN_STATUS')] == 1)): ?><td><div class="tl"><input id="status-<?php echo ($vo[C('DB_USSW_SALE_PLAN_ID')]); ?>" name="status-<?php echo ($vo[C('DB_USSW_SALE_PLAN_ID')]); ?>" type='checkbox' checked /></div></td>
						<?php else: ?> <td><div class="tl"><input id="status-<?php echo ($vo[C('DB_USSW_SALE_PLAN_ID')]); ?>" name="status-<?php echo ($vo[C('DB_USSW_SALE_PLAN_ID')]); ?>" type='checkbox' /></div></td><?php endif; ?>
						<td>
