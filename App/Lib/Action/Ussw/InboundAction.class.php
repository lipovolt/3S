	...
	public function index(){
        ...
        foreach ($usswInOrders as $key => $value) {
            $Data[$key]=array(
                ...
                C('DB_USSW_INBOUND_RECEIVE_DATE')=>$value[C('DB_USSW_INBOUND_RECEIVE_DATE')],
            ...
            if (!empty($_FILES)) {
                ...
                if($this->verifyImportedInboundItemTemplateColumnName($firstRow)){
                    ...
                    if($errorInFile != null){
                        ...
                        $restock->startTrans(); 
                        foreach ($data as $key => $value){
                            $r = $restock->where(array(C('DB_RESTOCK_ID')=>$value[C('DB_USSW_INBOUND_ITEM_RESTOCK_ID')]))->find();
                            if($r[C('DB_RESTOCK_QUANTITY')] <= $value[C('DB_USSW_INBOUND_ITEM_DQUANTITY')]){
                                $r[C('DB_RESTOCK_STATUS')] = '已发货';
                                $r[C('DB_RESTOCK_SHIPPING_DATE')] = date("Y-m-d H:i:s" ,time());
                                $restock->save($r);
                            }else{
                                $tmp[C('DB_RESTOCK_QUANTITY')] = $value[C('DB_USSW_INBOUND_ITEM_DQUANTITY')];
                                $tmp[C('DB_RESTOCK_STATUS')] = '已发货';
                                $tmp[C('DB_RESTOCK_SKU')] = $value[C('DB_USSW_INBOUND_ITEM_SKU')];
                                ...
                                $restock->add($tmp);
                                $r[C('DB_RESTOCK_QUANTITY')]= $r[C('DB_RESTOCK_QUANTITY')]-$value[C('DB_USSW_INBOUND_ITEM_DQUANTITY')];
        ...
        if($status != "已入库"){
            ...
            foreach ($items as $value) {
               ...
               if($this->isInStorage($value[C('DB_USSW_INBOUND_ITEM_SKU')])!=0){
                    $q[C('DB_PRODUCT_CNAME')] = $product->where(array(C('DB_PRODUCT_SKU')=>$value[C('DB_USSW_INBOUND_ITEM_SKU')]))->getField(C('DB_PRODUCT_CNAME'));
